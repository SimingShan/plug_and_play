...
base_model = MLP_NS()
base_model.load_state_dict(torch.load("./checkpoint/model_checkpoint.pth", map_location=device))
...
print(lora_model.lora_layers)
train(lora_model, x, y, u, v, p, Re, num_epochs, initial_lr, lora_model.lora_layers)
u_loss, v_loss, p_loss, f_loss, g_loss, continuity_loss = model_testing(lora_model, grid_size=100, Re=20, device=device)