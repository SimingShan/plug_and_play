...
train_data = torch.tensor(np.vstack((x_flat, y_flat)).T, dtype=torch.float32).to(device)
train_targets = torch.tensor(np.vstack((u.flatten(), v.flatten(), p.flatten())).T, dtype=torch.float32).to(device)
...
# Initialize base network and LoRA model
base_network = network()
model = LoRAWithHypernetwork(base_network, rank=1).to(device)
...
for epoch in range(10000):
    optimizer.zero_grad()
    reynolds = torch.tensor([[Re]], dtype=torch.float32).to(device)
    output = model(train_data, reynolds)
...
torch.save(model.state_dict(), 'LoRA_model_full_batch.pt')
plot_uvp(x_flat, y_flat, u, v, p, model, device, Re)
